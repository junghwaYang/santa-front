...
export function calculateResult(answers: AnswerOption[]) {
  ...
  const modifierScores: Record<ModifierType, number> = {
    SELF_CARE: 0,
    SOCIAL: 0,
    HOMEBODY: 0,
    EMOTIONAL: 0,
  ...
  // Find max
  const maxCharacter = (Object.keys(characterScores) as CharacterType[]).reduce((a, b) =>
    characterScores[a] > characterScores[b] ? a : b
  );
  const maxModifier = (Object.keys(modifierScores) as ModifierType[]).reduce((a, b) =>
    modifierScores[a] > modifierScores[b] ? a : b
  );
